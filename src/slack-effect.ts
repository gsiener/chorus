/**
 * Effect-based Slack API integration with typed errors and service pattern
 */
  ...
  return Effect.gen(function* () {
    ...
    if (!timestamp || !signature) {
      return yield* Effect.fail(
        new SignatureVerificationError("missing_headers")
      );
    }
    ...
    if (Math.abs(now - parseInt(timestamp)) > 5) {
      return yield* Effect.fail(
        new SignatureVerificationError("timestamp_expired")
      );
    }
    ...
    if (signature !== computedSignature) {
      return yield* Effect.fail(
        new SignatureVerificationError("signature_mismatch")
      );
    }
...
export function addReactionEffect(
  ...
  return Effect.gen(function* () {
    ...
    if (!data.ok && data.error !== "already_reacted") {
      return yield* Effect.fail(
        new SlackApiError(data.error ?? "unknown", "reactions.add")
      );
    }
...
export function postDirectMessageEffect(
  ...
  return Effect.gen(function* () {
    ...
    if (!openData.channel) {
      return yield* Effect.fail(
        new SlackApiError("no_channel_returned", "conversations.open")
      );
    }
